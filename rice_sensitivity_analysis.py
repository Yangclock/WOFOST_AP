...
def rice_sensitive(nutrition=True):
    # 路径设置
    ...
    # --------------------------------------------------------
    if nutrition:
        result_file_name = 'rice_result_npk.txt'
        lai_file_name = 'rice_LAI_result_npk.txt'
    else:
        result_file_name = 'rice_result_pp.txt'
        lai_file_name = 'rice_LAI_result_pp.txt'
    with open(os.path.join(para_dir, result_file_name), 'a') as fp3:
        fp3.writelines(['3', '\n', 'TAGP', '\n', 'TWSO', '\n', 'PuptakeTotal', '\n', 'time = no', '\n'])
        with open(os.path.join(para_dir, lai_file_name), 'a') as fp2:
            fp2.writelines(['1', '\n', 'LAI', '\n', 'time = yes', '\n'])
            #  打开simlab输出的文档
            with open(os.path.join(para_dir, 'EFAST_rice0112.SAM'), 'r') as fp:
                fp.readline()  # 第一行

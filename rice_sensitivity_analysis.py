...
def rice_1_sensitive(nutrition=True):
    # 路径设置
    work_dir = os.getcwd()
    ...
    variety_name = "Hubei_rice_1"  # 作物种类名称
    ...
    # --------------------------------------------------------
    if nutrition:
        result_file_name = 'rice1_result_npk.txt'
        lai_file_name = 'rice1_LAI_result_npk.txt'
    else:
        result_file_name = 'rice1_result_pp.txt'
        lai_file_name = 'rice1_LAI_result_pp.txt'
    with open(os.path.join(para_dir, result_file_name), 'a') as fp3:
        fp3.writelines(['3', '\n', 'TAGP', '\n', 'TWSO', '\n', 'PuptakeTotal', '\n', 'time = no', '\n'])
        with open(os.path.join(para_dir, lai_file_name), 'a') as fp2:
            fp2.writelines(['1', '\n', 'LAI', '\n', 'time = yes', '\n'])
            #  打开simlab输出的文档
            with open(os.path.join(para_dir, 'rice1.sam'), 'r') as fp:
                fp.readline()  # 第一行
                ...
                for i in range(int(number)):
                    ...
                    parameters['AMAXTB'][5] = sim_paraments[12]
                    parameters['AMAXTB'][9] = sim_paraments[13]
                    parameters['AMAXTB'][11] = sim_paraments[14]
                    parameters['TMPFTB'][3] = sim_paraments[15]
                    parameters['TMPFTB'][5] = sim_paraments[16]
                    parameters['FRTB'][1] = sim_paraments[26]
                    parameters['FRTB'][5] = sim_paraments[27]
                    parameters['FRTB'][7] = sim_paraments[28]
                    ...
                    parameters['FSTB'][7] = 0.7 - sim_paraments[31]
                    parameters['FSTB'][9] = 0.3 - sim_paraments[32]
...
def rice_2_sensitive(nutrition=True):
    # 路径设置
    work_dir = os.getcwd()
    para_dir = os.path.join(work_dir, "simlab_sensitivity_analysis")  # 敏感性分析参数读取与输出
    weather_dir = os.path.join(work_dir, "parameters", "meteorological_parameter")  # 气象数据路径
    ...
    change_data = {'TSUM1': 0, 'TSUM2': 1, 'TDWI': 2, 'RGRLAI': 3, 'SPAN': 6, 'TBASE': 7, 'CVL': 17, 'CVO': 18,
                   ...
                   'RDMCR': 39, 'WAV': 40, 'SMLIM': 41, 'NAVAILI': 42, 'PAVAILI': 43, 'KAVAILI': 44}
    crop_name = row['crop_name_summer']  # 作物名称
    variety_name = "Hubei_rice_2"  # 作物种类名称
    crop_data = YAMLCropDataProvider(crop_parameter_dir)  # 作物参数读取
    crop_data.set_active_crop(crop_name, variety_name)  # 设置当前活动作物
    soil_data = CABOFileReader(os.path.join(soil_parameter_dir, row['soil_file'] + '.new'))  # 土壤参数读取
    parameters = ParameterProvider(crop_data, soil_data,
    ...
    # --------------------------------------------------------
    if nutrition:
        result_file_name = 'rice2_result_npk.txt'
        lai_file_name = 'rice2_LAI_result_npk.txt'
    else:
        result_file_name = 'rice2_result_pp.txt'
        lai_file_name = 'rice2_LAI_result_pp.txt'
    with open(os.path.join(para_dir, result_file_name), 'a') as fp3:
        fp3.writelines(['3', '\n', 'TAGP', '\n', 'TWSO', '\n', 'PuptakeTotal', '\n', 'time = no', '\n'])
        with open(os.path.join(para_dir, lai_file_name), 'a') as fp2:
            fp2.writelines(['1', '\n', 'LAI', '\n', 'time = yes', '\n'])
            #  打开simlab输出的文档
            with open(os.path.join(para_dir, 'rice2.sam'), 'r') as fp:
                fp.readline()  # 第一行
                number = fp.readline()  # 第二行为生成参数个数
                fp.readline()  # 变量个数
                fp.readline()  # 0  此后开始读参数
...
if __name__ == '__main__':
    ...
    rice_1_sensitive(True)
    rice_1_sensitive(False)
    rice_2_sensitive(True)
    rice_2_sensitive(False)

...
def main(lat,lon):
    ...
    dataexcel.to_excel(r'D:\Desktop\WOFOST_AP\Meteorological_parameter\NASA天气文件lat={},lon={}.xlsx'.format(latitude, longitude),
...
if __name__ == '__main__':
    gewang = pd.read_excel(r'D:\Desktop\WOFOST_AP\Meteorological_parameter\剩余的.xlsx')
    columns = gewang.columns
    res = []
    for c in columns:
        d = gewang[c].tolist()
        res.append(d)
    for i in range(len(res[0])):
        main(st_loc(res[1][i]),st_loc(res[0][i]))
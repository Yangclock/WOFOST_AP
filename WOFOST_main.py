...
def enkf():
    # 路径设置
    work_dir = os.getcwd()
    data_dir = os.path.join(work_dir, "enkf")  # 工作路径
    ...
    for index, row in data_base_info.iterrows():  # 逐行读取点位信息并模拟
        ...
        if crop_name_winter == 'wheat_local':
            ...
            dates_of_observation = row[45:70].index.tolist()
            observed_lai = np.array(row[45:70])
        ...
        if variety_name_summer == 'Hubei_rice_1':
            ...
            # 添加观测数据
            variables_for_DA = ["LAI"]
            dates_of_observation = row[76:86].index.tolist()
            observed_lai = np.array(row[76:86])
            std_lai = observed_lai * 0.2  # MODIS LAI的不确定性大概有20%
            observations_for_DA = [(d, {"LAI": (lai, errlai)}) for d, lai, errlai in
                                   zip(dates_of_observation, observed_lai, std_lai)]
            # 定义集合中的变异性
        ...
        if variety_name_summer == 'Hubei_rice_2':
            ...
            # 添加观测数据
            variables_for_DA = ["LAI"]
            dates_of_observation = row[77:86].index.tolist()
            observed_lai = np.array(row[77:86])
            std_lai = observed_lai * 0.2  # MODIS LAI的不确定性大概有20%
            observations_for_DA = [(d, {"LAI": (lai, errlai)}) for d, lai, errlai in
                                   zip(dates_of_observation, observed_lai, std_lai)]
            # 定义集合中的变异性
        ...
        if int(index)/100 == 0:
            print(index)
    ap_summary_output.to_excel(os.path.join(re_dir, "EnKF最终结果.xlsx"))
